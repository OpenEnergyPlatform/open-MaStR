...
def get_output_dir() -> Path:
    """
    Return the directory used for CSV, XML and DB output.

    * By default the directory is ``$HOME/.open-MaStR``.
    * If the environment variable ``OUTPUT_PATH`` is defined, its value is used.
    ...
    """
    default_dir = Path.home() / ".open-MaStR"
    env_path = os.getenv("OUTPUT_PATH")
    if env_path:
        try:
            candidate = Path(env_path).expanduser().resolve(strict=False)
            return candidate
        except Exception as exc:
            print(
                f"Warning: could not use OUTPUT_PATH='{env_path}' ({exc}). "
                f"Falling back to default '{default_dir}'."
            )
    return default_dir
...
def get_data_version_dir():
    ...
    return get_output_dir() / "data" / data_version
...
def get_filenames():
    ...
    """
    with open(get_output_dir() / "config" / "filenames.yml") as filename_fh:
...
def create_project_home_dir():
    ...
    config_path = get_output_dir() / "config"
    log.info(f"I will create a default set of config files in {config_path}")

    internal_config_dir = os.path.join(Path(__file__).parent.absolute(), "config")
    files = ["logging.yml"]
...
def _filenames_generator():
    ...
    filenames_file = get_output_dir() / "config" / "filenames.yml"
...
def setup_project_home():
    ...
    Create PROJECTHOME returned by :func:`~.get_output_dir`.
...
def setup_logger():
    ...
    # Read logging config
    with open(get_output_dir() / "config" / "logging.yml") as filename_fh:
        logging_config = yaml.safe_load(filename_fh)

    # Add logfile location
    logging_config["handlers"]["file"]["filename"] = (
        get_output_dir() / "logs" / "open_mastr.log"
    )
