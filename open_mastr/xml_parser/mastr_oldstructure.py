...
class Mastr:
    ...
    def __init__(self, user_credentials={}, postgres_standard_credentials={}):
        ...
        ), "Variable user_credentials has to be a python dictionary!"
        ...
        initialize_database(
            ...
            postgres_standard_credentials=postgres_standard_credentials,
        )
            ...
            for name in f.namelist():
                ...
                if exclude_count == exclude_count_reference:
                    ...
                    if (
                        ...
                        if_exists = "replace"
                        print(
                            "New table %s is created in the PostgreSQL database."
                            % sql_tablename
                        )
                    ...
                    data = f.read(name)
                    save_path_metadata = os.path.join(
                        expanduser("~"), ".mastrsql", "metadata"
                    )
                    try:
                        df = pd.read_xml(data, encoding="UTF-16", compression="zip")
                        df, sql_dtype_dict = correction_of_metadata(df, sql_tablename)
                    ...
                    while continueloop:
                        try:
                            ...
                            print(
                                f"The entry {delete_entry} was deleteted due to its false data type."
                            )
