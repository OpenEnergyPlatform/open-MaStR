...
in `$HOME/.open-MaStR/data/sqlite`. With the function `Mastr.download()`, the MaStR is downloaded in the zipped xml file 
...
This method has the advantage, that single entries of the MaStR can be downloaded (one does not need to download the whole MaStR).
