...
-- Create a copy
DROP TABLE IF EXISTS model_draft.bnetza_mastr_rli_v2_1_2_wind_clean CASCADE;
CREATE TABLE         model_draft.bnetza_mastr_rli_v2_1_2_wind_clean AS
    SELECT  * 
    FROM    model_draft."bnetza_mastr_rli_v2.1.2_wind"
    ORDER BY id;

ALTER TABLE model_draft.bnetza_mastr_rli_v2_1_2_wind_clean
    ADD PRIMARY KEY (id),
...
CREATE INDEX bnetza_mastr_rli_v2_1_2_wind_clean_geom_idx
    ON model_draft.bnetza_mastr_rli_v2_1_2_wind_clean USING gist (geom);

-- clean lat and lon values
UPDATE  model_draft.bnetza_mastr_rli_v2_1_2_wind_clean
    SET lat =      CAST(
                        COALESCE(
                            NULLIF(
...
FROM model_draft.bnetza_mastr_rli_v2_1_2_wind_clean
LIMIT 10;
...
-- Check geom
UPDATE  model_draft.bnetza_mastr_rli_v2_1_2_wind_clean AS t1
...
-- Punkte innerhalb vg250
    ...
    FROM    (
        ...
        FROM    boundaries.bkg_vg250_1_sta_union_mview AS vg,
                model_draft.bnetza_mastr_rli_v2_1_2_wind_clean AS m
...
FROM model_draft.bnetza_mastr_rli_v2_1_2_wind_clean_50hertz
